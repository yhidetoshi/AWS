...
`aws ec2 --region ap-northeast-1 describe-instances | jq '.Reservations[].Instances[] | { RoleNmae: select(.InstanceId == "i-xxxxxxxxxxxxxxx") | (.Tags[] | select(.Key=="Role").Value)}' | jq -r '.RoleNmae'`
